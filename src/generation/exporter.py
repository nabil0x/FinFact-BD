...
from src.generation.utils import context_window, sentence_spans, write_json
...
class HumanValidationWorkbookBuilder:
    ...
    def build(self, samples: List[SampleRecord]) -> None:
        ...
        self._provenance(provenance, samples, Alignment, Font, PatternFill)
    ...
    def _instructions(self, ws: object, Alignment: object) -> None:
        rows = [
            ("Task", "Judge the rewritten claim_focus first. Decide whether it reads original, rewritten, or not sure."),
            ...
            ("Ratings", "Use 1-5 for plausibility and fluency/style. Mark context preservation and hallucination explicitly."),
    ...
    def _samples(self, ws: object, samples: List[SampleRecord], Alignment: object, Font: object, PatternFill: object, DataValidation: object) -> None:
        ...
        ws.append(headers)
        for sample in samples:
            claim_focus = self._target_sentence(sample.rewritten_article, int(sample.claim_index))
            ws.append(
                [
                    sample.sample_id,
                    sample.headline,
        ...
        rating_validation = DataValidation(type="whole", operator="between", formula1="1", formula2="5", allow_blank=True)
        yes_no_validation = DataValidation(type="list", formula1='"yes,no,not sure"', allow_blank=True)
        ...
        rating_validation.add(f"G2:H{max(ws.max_row, 2)}")
        yes_no_validation.add(f"I2:J{max(ws.max_row, 2)}")
        widths = {"A": 16, "B": 42, "C": 70, "D": 80, "E": 14, "F": 14, "G": 16, "H": 18, "I": 18, "J": 18, "K": 48}
    ...
    def _provenance(self, ws: object, samples: List[SampleRecord], Alignment: object, Font: object, PatternFill: object) -> None:
        ws.append(["sample_id", "original_target_claim", "rewritten_target_claim", "rewrite_plan", "verification_scores"])
        for sample in samples:
            ws.append(
                [
                    sample.sample_id,
                    sample.selected_claim.get("sentence", ""),
                    self._target_sentence(sample.rewritten_article, int(sample.claim_index)),
                    json.dumps(sample.rewrite_plan, ensure_ascii=False),
                    json.dumps(sample.verification_scores, ensure_ascii=False),
                ]
            )
        for cell in ws[1]:
            cell.font = Font(bold=True, color="FFFFFF")
    ...
    def _target_sentence(self, text: str, claim_index: int) -> str:
        spans = sentence_spans(text)
        if 0 <= claim_index < len(spans):
            return spans[claim_index].text
        return text.strip()
