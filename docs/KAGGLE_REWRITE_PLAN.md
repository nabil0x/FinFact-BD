...
Run the 8B models sequentially in 4-bit quantization on Kaggle. Qwen performs structured reasoning only; Aya performs Bangla realization only. The default config lazy-loads Qwen and Aya and releases them at role boundaries, so they are not resident on GPU at the same time. The generation model is for planned local realization, not for choosing misinformation or inventing articles.
